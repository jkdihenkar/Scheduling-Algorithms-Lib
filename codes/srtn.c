/*
    Problem Defination : Implementation of Shortest Remaining Time First (SRTN) Sched. algorith.
    ...
    Special Notes : SRTN Algorith Implementation
...
FILE *write;
void srtn(pptr* head,pptr* tail,const int count);
int main() {
...
//srtn algorithm -- no text referred developed from self logic : Jay Dihenkar
//date : 8th March 2014.
void srtn(pptr* head,pptr* tail,const int count) {
  ...
  while((topIndex>-1&&i<topIndex) || qindex>-1) { 
    ...
    //now decided that pqueue[j] is the shortest process...
      //serve it
      global_time+=1;
      pqueue[j]->burst_time-=1;
      printf("\nThe process %d is served for 1 slot.. Rem Time is : %d",pqueue[j]->pid,pqueue[j]->burst_time);
      if(pqueue[j]->burst_time==0) { 
