/*
    Problem Defination : Implementation of Priority-Non Pre-emptive Sched. algorith.
    ...
    Special Notes : Priority-Non Pre-emptive Algorith Implementation
...
FILE *write;
void priosimple(pptr* head,pptr* tail,const int count);
int main() {
...
void priosimple(pptr* head,pptr* tail,const int count) {
  ...
  while((topIndex>-1&&i<topIndex) || qindex>-1) {
    ...
    for(t=0;t<=qindex;t++) {
      ...
      global_time+=pqueue[rr]->burst_time;
      pqueue[rr]->burst_time=0;
